...
base_datasets_path = "/my_data/"
INPUT_SHAPE = (1292,96, 1)
label_list = ['car','chill','club','dance','gym','happy','morning','night','park','party','relax','running','sad',
              'shower','sleep','summer','train','training','work','workout']


def split_dataset(csv_path = os.path.join(base_datasets_path,"GroundTruth/ground_truth_single_label.csv"),
                  test_size=0.25, seed = 0, save_csv = True ,
                  train_save_path = os.path.join(base_datasets_path,"GroundTruth/train_ground_truth.csv"),
                  test_save_path = os.path.join(base_datasets_path,"GroundTruth/test_ground_truth.csv")):
...
def load_spectrogram(*args):
    ...
    """
    # TODO: edit path
    path = os.path.join(base_datasets_path,"/MelSpectograms_top20/")
    ...
    try:
        ...
        if (spect.shape != (1, 1292, 96)):
            # tf.logging.warn(f"Error while computing features for {song_id}")
...
def get_dataset(input_csv, input_shape=INPUT_SHAPE, batch_size=32, shuffle=True,
    ...
    dataset = dataset.map(lambda sample: dict(sample, binary_label=tf_multilabel_binarize(
        sample.get("label", b""), label_list_tf=tf.constant(label_list))[0]), )

    # set output shape
    dataset = dataset.map(lambda sample: dict(sample, binary_label=dp.set_tensor_shape(
        sample["binary_label"], (len(label_list)))))
...
def load_test_set_raw(LOADING_PATH = os.path.join(base_datasets_path,"GroundTruth/"),
    ...
    spectrograms = np.zeros([len(test_ground_truth), 1292, 96, 1])
...
def evaluate_model(model,spectrograms,test_classes):
    """
